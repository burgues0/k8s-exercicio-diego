...
export class ResponsaveisTecnicosService {
  ...
  async create(
    input: CreateResponsavelTecnicoDto,
  ...
  async update(
    id: number,
    input: UpdateResponsavelTecnicoDto,
    ...
    if (!hasChanges) {
      throw new BadRequestException(
        'Nenhuma alteração fornecida em relação aos dados atuais',
      );
    }
    ...
    if (input.registro_profissional !== undefined) {
      if (
        input.registro_profissional ===
        responsavelExistente.registro_profissional
      ) {
        throw new BadRequestException(
          'O registro profissional fornecido é igual ao atual',
        );
      }
      if (!input.registro_profissional.trim()) {
        throw new BadRequestException(
          'Registro profissional não pode ser vazio',
        );
      }
    }
    ...
    if (input.especialidade !== undefined) {
      if (input.especialidade === responsavelExistente.especialidade) {
        throw new BadRequestException(
          'A especialidade fornecida é igual à atual',
        );
      }
  ...
  async createVinculosObra(
    responsavelId: number,
    vinculosDto: CreateVinculoObraDto[],
    ...
    const vinculosDuplicados = obraIdsNovos.filter((id) =>
      obraIdsAtuais.has(id),
    );
    ...
    for (const dto of vinculosDto) {
      const vinculo = await this.processarCriacaoVinculo(
        responsavelId,
        dto,
        vinculosAtuais,
      );
  ...
  async updateVinculoObra(
    responsavelId: number,
    obraId: number,
    updateDto: UpdateVinculoObraDto,
  ): Promise<ObraResponsavelTecnico> {
    // Validações básicas
    this.validarId(responsavelId);
    ...
    const vinculoExistente = await this.obterVinculoExistente(
      responsavelId,
      obraId,
    );
    ...
    const hasChanges = Object.keys(updateDto).some((key) => {
      ...
      const existingValue =
        vinculoExistente[key === 'tipoVinculo' ? 'tipo_vinculo' : key];
  ...
  async findVinculoObra(
    ...
    const vinculo = await this.obrasResponsavelTecnicoRepository.buscarVinculo(
      responsavelId,
      obraId,
    );
    if (!vinculo) {
      throw new NotFoundException(
        `Vínculo entre responsável ${responsavelId} e obra ${obraId} não encontrado.`,
      );
    }
  ...
  async deleteVinculoObra(
    ...
    try {
      await this.obrasResponsavelTecnicoRepository.removerVinculo(
        responsavelId,
        obraId,
      );
      ...
      throw new InternalServerErrorException(
        'Erro ao remover o vínculo. Por favor, tente novamente.',
      );
    }
  }
  ...
  private async obterResponsavelPorId(id: number): Promise<ResponsavelTecnico> {
    ...
    if (!responsavel) {
      throw new NotFoundException(
        `Responsável técnico com id ${id} não encontrado.`,
      );
    }
    return responsavel;
  }
  ...
  private async verificarCPFUnico(
    cpf: string,
    idExcluir?: number,
  ): Promise<void> {
    const responsavelExistente =
      await this.responsaveisTecnicosRepository.findByCPF(cpf);
  ...
  private async verificarVinculosAtivos(idResponsavel: number): Promise<void> {
    ...
    const vinculosAtivos = vinculos.filter(
      (vinculo) => !vinculo.data_fim || new Date(vinculo.data_fim) >= hoje,
    );

    if (vinculosAtivos.length > 0) {
      throw new ConflictException(
        'Não é possível remover um responsável com vínculos ativos.',
      );
    }
  }

  private async processarCriacaoVinculo(
    responsavelId: number,
    dto: CreateVinculoObraDto,
    vinculosAtuais: ObraResponsavelTecnico[],
  ): Promise<ObraResponsavelTecnico> {
    ...
    const { dataInicio, dataFim } = this.processarEValidarDatas(
      dto.dataInicio,
      dto.dataFim,
    );

    // Validações de negócio
    const tipoVinculo = this.validarTipoVinculo(
      dto.tipoVinculo as TipoVinculoObra,
    );
    this.verificarVinculoAtivo(vinculosAtuais, dto.obraId, tipoVinculo);
    this.verificarConflitoDatas(
      vinculosAtuais,
      dto.obraId,
      tipoVinculo,
      dataInicio,
  ...
  private validarTipoVinculo(tipoVinculo: string): TipoVinculoObra {
    if (
      !Object.values(TipoVinculoObra).includes(tipoVinculo as TipoVinculoObra)
    ) {
      throw new BadRequestException(
  ...
  private verificarVinculoAtivo(
    ...
    const jaVinculado = vinculosAtuais.some(
      (v) =>
        v.obra.id === obraId &&
  ...
  private verificarConflitoDatas(
    ...
    if (conflito) {
      throw new BadRequestException(
        `Existe sobreposição de datas com outro vínculo do tipo ${tipoVinculo} na obra ${obraId}.`,
      );
    }
  }
  ...
  private async criarNovoVinculo(
    responsavelId: number,
    dto: CreateVinculoObraDto,
    dataInicio: Date,
    dataFim: Date | null,
  ...
  private processarDatasAtualizacao(
    dataInicioDto: string | undefined,
    dataFimDto: string | null | undefined,
    vinculoExistente: ObraResponsavelTecnico,
  ): { novaDataInicio: Date; novaDataFim: Date | null } {
  ...
  private async obterVinculoExistente(
    responsavelId: number,
    obraId: number,
  ): Promise<ObraResponsavelTecnico> {
    const vinculo = await this.obrasResponsavelTecnicoRepository.buscarVinculo(
  ...
  private async verificarConflitosAtualizacao(
    responsavelId: number,
    obraId: number,
    novaDataInicio: Date,
    novaDataFim: Date | null,
    tipoVinculo: TipoVinculoObra,
    vinculoIdAtual: number,
  ): Promise<void> {
    const vinculosAtuais =
      await this.obrasResponsavelTecnicoRepository.buscarVinculosPorResponsavel(
        responsavelId,
    ...
    if (conflito) {
      throw new BadRequestException(
        'Existe sobreposição de datas com outro vínculo do mesmo tipo para esta obra.',
      );
    }
  }

  private async executarAtualizacao(
    responsavelId: number,
    obraId: number,
    dataInicio: Date,
    dataFim: Date | null,
    ...
    const resultado =
      await this.obrasResponsavelTecnicoRepository.atualizarVinculo(
        responsavelId,
        obraId,
        dadosAtualizacao,
      );
